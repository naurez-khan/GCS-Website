...
const downloadLectureStatement = async (req, res) => {
    try {
        ...
        const [courseResult, studentsResult, attendanceResult] = await Promise.all([
            pool.query(
                `SELECT c.id, c.name, c.course_code, c.class_type, c.intermediate_year,
                        c.class_shift, c.roll_number_type, c.program, c.semester, c.section,
        ...
        const pdfBuffer = await createLectureStatementPdf(spec);
        const safeName = String(course.course_code || course.name || "course")
            .replace(/[^a-z0-9_-]+/gi, "-")
            .replace(/^-+|-+$/g, "") || "course";

        res.setHeader("Content-Type", "application/pdf");
        res.setHeader("Content-Disposition", `attachment; filename="lecture-statement-${safeName}-${spec.endDate}.pdf"`);
        res.setHeader("Content-Length", pdfBuffer.length);
        return res.send(pdfBuffer);
    } catch (error) {
        console.error("Lecture statement error:", error);
        const isEmpty = /No saved attendance/.test(error.message || "");
        return res.status(isEmpty ? 400 : 500).json({
            success: false,
            message: isEmpty ? error.message : "Could not create the lecture statement"
        });
    }
};
